...
import Map, { TAGS } from '@/components/mapContainer';
...
export default function ({ logIn, setLogIn }: loginProps) {
  ...
  const changeLogIn = () => { setLogIn(false); }
  ...
  const handleFloorOpt    = () => { setFloorOptions(prev => !prev); }
  const toCVS             = () => { setIsBiomed(false); setFloorOptions(false); }
  const changeFloor       = () => { setIsBiomed(true);  setFloorOptions(false); }
  const showSystemSubSec  = () => { setSysSubSec(prev => !prev); }
  const handleShowTest    = () => { setShowTest(prev => !prev); setShowCalibration(false); }
  const handleShowCalibration = () => { setShowCalibration(prev => !prev); }
  ...
  return (
    <div className={`${logIn ? '' : 'hidden'} w-full h-screen bg-stone-50 text-[1rem] text-stone-800 flex flex-col`}>
      ...
      <div className="bg-stone-300 w-full flex-1 relative flex flex-col items-center px-6">
        ...
        {/* Search */}
        <form className={`shadow-stone-500 absolute items-center top-2 z-20 ${(showTest || showCalibration || showHistory) ? 'hidden' : 'flex'}`}>
        ...
        {/* SIDEBAR */}
        <div className={`z-20 h-full bg-stone-100 absolute flex flex-col left-0 py-[1.125rem] px-[1.5rem] transform transition-all duration-300 ease-in-out ${showSidebar ? 'w-[16.25rem]' : 'w-20'}`}>
          ...
          <div className='w-full flex flex-col'>
            ...
            {/* BLE-tracked devices — clickable */}
            <div className={`w-full text-[0.875rem] flex flex-col gap-[0.25rem] py-2 ${showSidebar ? '' : 'hidden'}`}>
              {TAGS.map(tag => (
                <section
                  key={tag.id}
                  onClick={() => setSelectedTagId(tag.id)}
                  className={`w-full flex justify-between items-center px-2 py-1 rounded-md cursor-pointer transition-colors
                    ${selectedTagId === tag.id ? 'bg-green-100' : 'hover:bg-stone-200'}`}
                >
                  <span className={`leading-[1] ${selectedTagId === tag.id ? 'text-green-600 font-semibold' : ''}`}>
                    {tag.name}
                  </span>
                  <span className='font-medium text-stone-700 text-[0.7rem] leading-[1]'>{tag.serial}</span>
                </section>
              ))}

              {/* Untracked — visible but dimmed */}
              {UNTRACKED_DEVICES.map(name => (
                <section key={name} className='w-full flex justify-between items-center px-2 py-1 opacity-40'>
                  <span className='leading-[1]'>{name}</span>
                  <span className='font-medium text-stone-700 text-[0.7rem] leading-[1]'>—</span>
                </section>
              ))}
            </div>
          </div>
          ...
          {/* HISTORY */}
          <div className={`flex items-center hover:bg-stone-300 rounded-md`} onClick={handleShowHistory}>
            <button className="p-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 text-stone-800">
          ...
          {/* NOTIFICATION */}
          <div className={`flex items-center hover:bg-stone-300 rounded-md`} onClick={handleShowNotification}>
            <button className="p-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 text-stone-800">
            ...
            <p className={`font-semibold text-[0.875rem] text-stone-800 ${showSidebar ? '' : 'hidden'}`}>Notification</p>
          </div>

          {/* SETTINGS */}
          <div className={`w-full flex flex-col justify-between ${showSidebar ? '' : 'hidden'}`}>
            <div className='flex items-center hover:bg-stone-300 rounded-md'>
              <button className="p-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 text-stone-800">
              ...
              <span className='font-semibold text-[0.875rem] text-stone-800'>Settings</span>
            </div>
            <div>
        ...
        {!(showTest || showCalibration || showHistory) && (
          <div className="w-full z-10 flex-1 flex items-center justify-center">
            <Map
              isBiomed={isBiomed}
              setIsBiomed={setIsBiomed}
              selectedTagId={selectedTagId}
            />
          </div>
        )}

        {showTest && <div className="h-full"><Test /></div>}
        {showCalibration && <div className="h-full w-[50rem]"><Calibration /></div>}
        {showHistory && <div className="h-full"><History /></div>}
        {showNotification && <div className="h-full"><Notification /></div>}
        ...
        <div className={`z-20 right-0 bottom-0 px-4 py-6 flex flex-col absolute gap-2 ${showTest ? 'hidden' : ''}`}>
          <section className='flex gap-2 justify-end items-center text-[0.875rem]'>
            <p>FLOOR</p>
            <button onClick={handleFloorOpt} className='w-[3rem] h-[3rem] bg-floor bg-cover bg-center rounded-full hover:cursor-pointer'></button>
