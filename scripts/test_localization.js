...
const DATA_PATH = path.join(__dirname, '..', 'src', 'data', 'recordedData.json');
const OUTPUT_DIR = path.join(__dirname, '..', 'test_output');
if (!fs.existsSync(OUTPUT_DIR)) fs.mkdirSync(OUTPUT_DIR, { recursive: true });

const trainingData = JSON.parse(fs.readFileSync(DATA_PATH, 'utf8'));

// Helper: Box-Muller for normal samples
function randn_bm() {
  let u = 0, v = 0;
...
// Generate synthetic tests: at least 20 per zone
...
for (const test of syntheticTests) {
  ...
  if (pi >= 0) matrix[ti][pi] += 1; else {
    // unknown predicted id - ignore for matrix but still log
  }
  results.push({ trueId, predictedId, sample: test.sample });
}

// Save results
fs.writeFileSync(path.join(OUTPUT_DIR, 'synthetic_test_results.json'), JSON.stringify(results, null, 2));

// Save confusion matrix CSV: rows = true, cols = predicted
const header = ['true\\pred', ...zoneIds].join(',') + '\n';
const rows = matrix.map((row, idx) => [zoneIds[idx], ...row].join(',')).join('\n');
fs.writeFileSync(path.join(OUTPUT_DIR, 'confusion_matrix.csv'), header + rows);

console.log(`Done. Generated ${syntheticTests.length} synthetic tests (${TESTS_PER_ZONE} per zone).`);
